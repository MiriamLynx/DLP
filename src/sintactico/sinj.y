...
programa: listaDefiniciones VOID MAIN '(' ')' '{' listaCampos listaSentencias '}'	{ this.ast = new Programa(lexico.getLinea(), lexico.getColumna(), (List<Sentencia>)$8, (List<DefVariable>)$7, (List<DefVariable>)$1); }
        ;
...
defStruct: STRUCT '{' listaDefVariables '}' ID PUNTOYCOMA	{ $$ = new DefStruct(lexico.getLinea(), lexico.getColumna(),(String)$5, (List<DefVariable>)$3);}
		 ;
...
defVariable: tipo listaIds PUNTOYCOMA	{ $$ = new DefVariable(lexico.getLinea(), lexico.getColumna(), (List<String>)$2, (Tipo) $1);}
		   ;
...
listaExpresiones: expresion							{ List<Expresion> e= new ArrayList<Expresion>(); e.add((Expresion)$1); $$ = e;  }	
				| listaExpresiones COMA expresion	{ List<Expresion> e = (List<Expresion>)$1;e.add((Expresion)$3); $$ = e; }
				;
...
expresion: '(' expresion ')'			{ $$ = $2; }
		 ...
		 | expresion '.' ID 			{ $$ = new AccesoCampo(lexico.getLinea(), lexico.getColumna(), (String)$3, (Expresion)$1); }
...
sentencia:	WRITE listaExpresiones PUNTOYCOMA		{ $$ = new Escritura(lexico.getLinea(), lexico.getColumna(),(List<Expresion>)$2);}
		 |	READ listaExpresiones PUNTOYCOMA		{ $$ = new Lectura(lexico.getLinea(), lexico.getColumna(),(List<Expresion>)$2);}
		 ...
		 |  RETURN expresion PUNTOYCOMA				{ $$ = new Return(lexico.getLinea(), lexico.getColumna(),(Expresion)$2); }
		 |	expresion '=' expresion	PUNTOYCOMA		{ $$ = new Asignacion(lexico.getLinea(), lexico.getColumna(),(Expresion)$1,(Expresion)$3);}
		 | ID '(' argumentos ')' 	PUNTOYCOMA		{ $$ = new LlamadaProcedimiento(lexico.getLinea(), lexico.getColumna(),(String)$1,(List<Expresion>)$3);}					
		 ;
...
parametros:	parametros COMA parametro	{ List<DefVariable> df = (List<DefVariable>)$1; df.add((DefVariable)$3); $$ = df ;}
		  | parametro					{ List<DefVariable> dv = new ArrayList<DefVariable>(); dv.add((DefVariable)$1); $$=dv; }
		  ;
		
parametro: tipo ID	{ List<String> s = new ArrayList<String>(); s.add((String)$2); DefVariable v = new DefVariable(lexico.getLinea(), lexico.getColumna(), s, (Tipo) $1); $$ = v;	}
		 ;

listaIds: listaIds COMA ID	{ List<String> st = (List<String>)$1; st.add((String)$3); $$ = st ;}
